...
final class MUTQAN_Analytics {
 ...
 public static function summary($from,$to){
  ...
  $tech=$wpdb->get_results("SELECT u.ID id,u.display_name,COUNT(o.id) active_orders FROM $users u INNER JOIN {$wpdb->usermeta} um ON um.user_id=u.ID AND um.meta_key='wp_capabilities' LEFT JOIN $ops o ON o.technician_id=u.ID AND o.status IN ('assigned','accepted','en_route','nearby','arrived','working','waiting_customer') WHERE um.meta_value LIKE '%mutqan_technician%' GROUP BY u.ID,u.display_name ORDER BY active_orders DESC LIMIT 50",ARRAY_A);
